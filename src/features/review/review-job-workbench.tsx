...
import { formatCategoryLabel } from "@/lib/taxonomy-display";
...
export function ReviewJobWorkbench({ detail }: ReviewJobWorkbenchProps) {
  ...
  const canConfirm =
    ...
    !invalidQuestion &&
    !missingMergeTarget;
  const disabledConfirmReason = !canConfirm
    ? jobSummary.job_type !== "extract_interview"
      ? "当前任务类型还没有确认写入路径。"
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.1fr)_minmax(0,0.9fr)] xl:[--review-panel-height:min(760px,calc(100vh-15rem))] xl:items-stretch">
        <SurfaceCard className="space-y-5 xl:flex xl:h-[var(--review-panel-height)] xl:flex-col">
          ...
          <div className="space-y-5 xl:min-h-0 xl:flex-1 xl:overflow-y-auto xl:pr-1">
            ...
            <FormField label="原文">
              <Textarea
                className="min-h-[280px] font-mono text-xs leading-6 xl:min-h-[460px]"
                readOnly
