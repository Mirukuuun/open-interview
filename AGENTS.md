...
- 本应用为 local-first 单用户工具，**不包含任何认证/授权机制**。API 路由对所有网络可达的调用方完全开放。
- 生产部署**必须**通过反向代理（Caddy）限制为仅 `127.0.0.1` 回环访问或受信任的内网，不得将 Next.js 服务直接暴露于公网。
- LLM 相关路由（`/api/qa/*/ask`、`/api/parse-jobs`、`/api/practice/*/submit`）会触发外部 API 调用，无速率限制；若需公网暴露，须先添加认证中间件和速率限制。
