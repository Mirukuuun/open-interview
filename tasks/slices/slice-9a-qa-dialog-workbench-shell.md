...
## Done when
- [ ] `/qa` 与 `/qa/:sessionId` 呈现为统一视觉与布局体系
- [ ] 左栏能稳定展示 recent sessions，并高亮当前 session
- [ ] `/qa` 空态可直接提问，首问后创建 session 并进入 `/qa/:sessionId`
- [ ] 聊天 transcript 成为主阅读区域，输入框位置符合连续提问习惯
- [ ] citations / related questions / retrieval summary 有稳定位置，但不抢主体验
- [ ] `strategy` / `top_k` 不再作为普通用户默认主操作项
