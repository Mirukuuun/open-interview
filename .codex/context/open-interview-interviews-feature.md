...
1. 列表页按公司、岗位、轮次或关键词浏览面经来源，并按公司分组暴露沉淀完成度。
2. 详情页聚合 source 元数据、摘要、面经原题、上下文片段以及每道面经题的题库推荐与正式沉淀结果，同时在首屏展示沉淀概览与继续沉淀 CTA。
...
- 面经列表应能快速区分 `待沉淀`、`部分沉淀`、`已沉淀`，避免用户只能进入详情后再判断处理优先级。
