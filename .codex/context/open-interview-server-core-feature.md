...
- 主要目录：`src/server/api`、`src/server/db`、`src/server/repositories`、`src/server/services`、`src/server/retrieval`、`src/server/prompts`
...
2. service 通过 repository、storage、retrieval 和集中 prompt 模板协同完成用例。
...
- parse / QA / practice grading 的固定 LLM prompt 模板统一维护在 `src/server/prompts`，业务文件只负责拼装运行时上下文并发起调用。
