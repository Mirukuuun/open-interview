...
2. 详情页读取问题主体、主答案、补充视角、来源关联、手动沉淀的面经题来源和标签信息，并根据当前浏览上下文提供上一条 / 下一条跳转与基于当前题目的 QA 入口。
...
- 题库详情发起 QA 默认只预填当前题干，保持 query prefill 入口轻量可复用。
- 题库 UI 对 canonical `category` / `tag` key 做中文显示映射，但筛选参数和底层存储仍保留原始 key。
- 面经题不会自动进入题库；只有手动沉淀后的 `interview_question_link` 才会在题库详情中作为来源面经题展示。
