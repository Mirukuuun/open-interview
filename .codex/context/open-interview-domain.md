...
- `assessment_session` / `assessment_item`：随机练习中的 10 题考试快照、用户答案与评分结果。
...
- 模拟考试只从带 `canonical_answer` 的 active 题目中抽题，保证评分有稳定参照。
...
4. 题库中的 active 问题可进入随机练习，带标准答案的问题可进入模拟考试评分。
5. 基于 canonical 实体与 `chunk` / `embedding` 进行搜索和 grounded QA。

## 领域术语

- Review Queue：待人工确认的解析任务集合。
- Question Bank：确认后的题库主视图。
- Random Practice：围绕题库的快速刷题与 10 题考试工作台。
